...
app.get('/optiontarget', (req, res) => {
  res.header("Access-Control-Allow-Origin", "*");
  fundSpider.OptionFundData((data)=>{
    res.send(data); 
  })
});
app.post('/rules', (req, res) => {
  req.on('data',data=>{
    ...
    console.log(rules)
    fundSpider.RisingOneweek = Number(rules.RisingFund['oneweek']?rules.RisingFund['oneweek']:-40);
    fundSpider.RisingOnemonth = Number(rules.RisingFund['onemonth']?rules.RisingFund['onemonth']:-40);
    fundSpider.RisingThreemonth = Number(rules.RisingFund['threemonth']?rules.RisingFund['threemonth']:-40);
    fundSpider.RisingSixmonth = Number(rules.RisingFund['sixmonth']?rules.RisingFund['sixmonth']:-40);
    fundSpider.RisingOneyear = Number(rules.RisingFund['oneyaer']?rules.RisingFund['oneyaer']:-40);
    fundSpider.RisingThreeyear = Number(rules.RisingFund['threeyear']?rules.RisingFund['threeyear']:-40);
    fundSpider.FallingOneweek = Number(rules.FallingFund['oneweek']?rules.FallingFund['oneweek']:-40);
    fundSpider.FallingOnemonth = Number(rules.FallingFund['onemonth']?rules.FallingFund['onemonth']:-40);
    console.log(fundSpider);
  })
  res.header("Access-Control-Allow-Origin", "*");
  res.send('success');
});
app.post('/addoption', (req, res) => {
  ...
  req.on('data',data=>{
    ...
    if(fundSpider.optionFundCode.includes(code)){
      res.send('repeat');
    }else{
      fundSpider.optionFundCode.push(code);
      console.log(fundSpider.optionFundCode);
      fundSpider.OptionFundData((data)=>{
        res.send(data);
      });
    }
...
app.post('/removeoption', (req, res) => {
  res.header("Access-Control-Allow-Origin", "*");
  req.on('data',data=>{
    let code = data.toString();
    fundSpider.optionFundCode.splice(fundSpider.optionFundCode.findIndex(item =>{
      return item == code
    }),1)
    fundSpider.optionFundAll.splice(fundSpider.optionFundAll.findIndex(item =>{
      return item.code == code
    }),1)
    res.send(fundSpider.optionFundAll);
  })
});
app.listen(1234, () => {
...
class FundSpider {
  ...
  async OptionFundData(callback){
    this.optionFundAll = [];
    this.optionFundCode.forEach((code) => {
      this.fetchFundCodeInfo(code,(err,data)=>{
        if(!err){
          let oneweekAmout = Number(data.stageAmout['近1周'].replace(/%/g, ''));
          // console.log('代码：', code, '近一周涨幅：', oneweekAmout);
  ...
  FilterFund(data, code) {
    let oneweekAmout = Number(data.stageAmout['近1周'].replace(/%/g, ''));
    let onemonthAmout = Number(data.stageAmout['近1月'].replace(/%/g, ''))
    let threemonthAmout = Number(data.stageAmout['近3月'].replace(/%/g, ''));
    let sixmonthAmout = Number(data.stageAmout['近6月'].replace(/%/g, ''));
    let oneyearAmout = Number(data.stageAmout['近1年'].replace(/%/g, ''));
    let threeyearAmout = Number(data.stageAmout['近3年'].replace(/%/g, ''));
    
    if (oneweekAmout > 0 && onemonthAmout > this.RisingOnemonth && threemonthAmout > this.RisingThreemonth && sixmonthAmout > this.RisingSixmonth && oneyearAmout > this.RisingOneyear && threeyearAmout > this.RisingThreeyear ) {
      var str = data.assetScale;
      var patt1 = /^(-?\d+)(\.\d+)/g;
      var num = str.match(patt1);
      if (parseFloat(num) > 10) {
        console.log('代码：', code, '近一月涨幅：', onemonthAmout);
        this.upFundArr.push({
          code: code,
          Trend: onemonthAmout + '%',
          fundName: data.fundName,
    ...
    if (oneweekAmout < this.FallingOneweek && onemonthAmout < this.FallingOnemonth&& oneyearAmout > 50) {
      var str = data.assetScale;
      var patt1 = /^(-?\d+)(\.\d+)/g;
      var num = str.match(patt1);
      if (parseFloat(num) > 10) {
        console.log('代码：', code, '近一周下跌：', oneweekAmout);
